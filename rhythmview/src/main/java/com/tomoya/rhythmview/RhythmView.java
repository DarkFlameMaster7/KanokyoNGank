    ...
    * */
    ...
    private void makeItems(int position, List<View> viewList) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            int translationY = Math.min(Math.max(Math.abs(position - i) * mIntervalHeight, 10), mMaxTranslationHeight);
            //位移动画
    ...
    private int getFirstVisibleItemPosition() {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (((float) getScrollX()) < this.mLinearLayout.getChildAt(i).getX() + (this.mItemWidth / 2.0f))
                return i;
        }
        return 0;
    }
    ...
    * */
    ...
    class ShiftMonitorTimer extends Timer {
        ...
        void monitorTouchPosition(float x, float y) {
            ...
            if ((x < 0.0F) || ((x > mEdgeSizeForShiftRhythm) && (x < mScreenWidth - mEdgeSizeForShiftRhythm)) || (y <
                    0.0F)) {
                mFingerDownTime = System.currentTimeMillis();
                this.canShift = false;
            } else {
